...
export default function CounsellorDashboard() {
  ...
  const [form,setForm] = useState({course:COURSES[0],internUid:""});
  ...
  const assignVisit = async () => {
    if(!form.internUid){ showToast("Please select an intern"); return; }
    setSubmitting(true);
    const intern=interns.find(i=>i.uid===form.internUid);
    const vRef=push(ref(db,"visits"));
    await set(vRef,{studentName:"",phone:"",course:form.course,internUid:form.internUid,internName:intern?.name||"",counsellorUid:profile?.uid,counsellorName:profile?.name,status:"assigned",checkpoints:[],createdAt:Date.now()});
    await set(push(ref(db,`notifications/${form.internUid}`)),{title:"New Visit Assigned",message:`A new ${form.course} visit has been assigned to you by ${profile?.name}.`,read:false,createdAt:Date.now()});
    showToast(`Visit assigned to ${intern?.name}`);
    setForm({course:COURSES[0],internUid:""});
    setSubmitting(false);
    setTab("visits");
  };
      ...
      <div className="f" style={{display:"flex",minHeight:"100vh",background:"#08060f"}}>
          ...
          <div style={{borderTop:"1px solid #141414",paddingTop:12}}>
            <div style={{display:"flex",alignItems:"center",gap:8,padding:"6px 10px",marginBottom:6}}>
              ...
              <div style={{overflow:"hidden"}}>
                ...
                <p style={{fontSize:10,color:"#9d8cbb",margin:0}}>Counsellor</p>
              </div>
            </div>
        ...
        <div style={{flex:1,marginLeft:0,display:"flex",flexDirection:"column"}}>
          <header style={{padding:"12px 16px",borderBottom:"1px solid #141414",position:"sticky",top:0,background:"#08060f",zIndex:30,display:"flex",alignItems:"center",gap:12}}>
            ...
            <p style={{fontSize:13,fontWeight:500,color:"#b09fd0",margin:0,flex:1}}>{NAV.find((n:any)=>n.id===tab)?.label}</p>
          ...
          <main style={{flex:1,padding:"24px",overflowY:"auto"}}>
            ...
            {tab==="overview" && (
              <div style={{display:"flex",flexDirection:"column",gap:20}}>
                <div className="au">
                  ...
                  <p style={{fontSize:13,color:"#9d8cbb",margin:0}}>Manage campus visits</p>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
                  {[{l:"Total Visits",v:visits.length},{l:"Completed",v:visits.filter((v:any)=>v.status==="completed").length},{l:"On Duty",v:onDuty.length}].map((s,i)=>(
                    <div key={i} className="card au1" style={{padding:"16px 18px"}}>
                      ...
                      <p style={{fontSize:12,color:"#9d8cbb",margin:0}}>{s.l}</p>
                    </div>
                  ))}
                </div>
                <div className="au2 card" style={{overflow:"hidden"}}>
                  ...
                  {onDuty.length===0 ? <p style={{padding:"20px 16px",color:"#6b5a88",fontSize:13,margin:0}}>No interns on duty.</p>
                  : onDuty.map((u,i)=>(
            ...
            {tab==="assign" && (
              <div style={{display:"flex",flexDirection:"column",gap:16,maxWidth:480}}>
                <div className="au">
                  <p style={{fontSize:18,fontWeight:600,color:"#fff",margin:"0 0 2px"}}>Assign Visit</p>
                  <p style={{fontSize:13,color:"#9d8cbb",margin:0}}>Select a course and an on-duty intern — the intern will fill in student details</p>
                </div>
                <div className="au1 card" style={{padding:20}}>
                  <p style={{fontSize:11,fontWeight:500,color:"#9d8cbb",margin:"0 0 8px",letterSpacing:"0.5px",textTransform:"uppercase"}}>Course</p>
                  ...
                  <p style={{fontSize:11,fontWeight:500,color:"#9d8cbb",margin:"0 0 8px",letterSpacing:"0.5px",textTransform:"uppercase"}}>Select Intern</p>
                  {onDuty.length===0 ? <p style={{fontSize:13,color:"#6b5a88",marginBottom:16}}>No interns on duty right now.</p>
                  : onDuty.map(u=>(
                    <div key={u.uid} className={`intern-opt${form.internUid===u.uid?" sel":""}`} onClick={()=>setForm(f=>({...f,internUid:u.uid}))}>
                      <Av name={u.name}/>
                      <p style={{fontSize:13,fontWeight:500,color:"#f0ecff",margin:0,flex:1}}>{u.name}</p>
            ...
            {tab==="visits" && (
              <div style={{display:"flex",flexDirection:"column",gap:16}}>
                <div className="au"><p style={{fontSize:18,fontWeight:600,color:"#fff",margin:0}}>Live Visits</p></div>

                {/* Active visits */}
                {visits.filter((v:any)=>v.status!=="completed").length > 0 && (
                  <div className="au1">
                    <p style={{fontSize:11,fontWeight:500,color:"#9d8cbb",margin:"0 0 8px",letterSpacing:"0.5px",textTransform:"uppercase"}}>Active Now</p>
                    {visits.filter((v:any)=>v.status!=="completed").map((v:any,i:number)=>(
                      <div key={i} className="card" style={{padding:"14px 16px",marginBottom:8}}>
                        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                          <div style={{display:"flex",alignItems:"center",gap:6}}>
                            <div className="dot" style={{width:6,height:6,borderRadius:"50%",background:"#22c55e",flexShrink:0}}/>
                            <p style={{fontSize:13,fontWeight:600,color:"#fff",margin:0}}>{v.studentName||"Student details pending"}</p>
                          </div>
                          <Badge color={v.status==="started"?"amber":v.status==="in_progress"?"amber":"purple"}>{v.status}</Badge>
                        </div>
                        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
                          <Av name={v.internName} size={22}/>
                          <p style={{fontSize:12,color:"#c4b5fd",margin:0}}>{v.internName} <span style={{color:"#6b5a88"}}>·</span> {v.course}</p>
                        </div>
                        {v.currentCheckpoint && (
                          <div style={{display:"flex",alignItems:"center",gap:4,background:"#0a0810",borderRadius:6,padding:"5px 8px"}}>
                        ...
                        {v.checkpoints?.length>0 && (
                          <p style={{fontSize:10,color:"#6b5a88",margin:"4px 0 0"}}>Route: {v.checkpoints.map((c:string)=>CP_LABELS[c]||c).join(" → ")}</p>
                        )}
                      </div>
                    ))}
                  </div>
                ...
                <div className="au2 card" style={{overflow:"hidden"}}>
                  <div style={{padding:"12px 16px",borderBottom:"1px solid #141414"}}>
                    <p style={{fontSize:13,fontWeight:500,color:"#fff",margin:0}}>All Visits <span style={{color:"#9d8cbb"}}>({visits.length})</span></p>
                  </div>
                  {visits.length===0 ? <p style={{padding:"20px 16px",color:"#6b5a88",fontSize:13,margin:0}}>No visits yet.</p>
                  : visits.map((v:any,i:number)=>(
                    <div key={i} className="row">
                      <div style={{flex:1}}>
                        <p style={{fontSize:13,fontWeight:500,color:"#f0ecff",margin:0}}>{v.studentName||"—"} <span style={{color:"#6b5a88",fontSize:11}}>with {v.internName}</span></p>
